...
public class Launcher {
    ...
    public void launch(final Component comp) {
        ...
        final List<String> jars = getJars(new File(system.installDir, "libraries"));
        jars.add(new File(system.installDir, "minecraft.jar").getAbsolutePath());
        final List<String> cmd = new LinkedList<String>();
        cmd.add("java");
        cmd.add("-cp");
        cmd.add(String.join(":", jars));
        cmd.add("-Djava.library.path=".concat(new File(system.installDir, "natives").getAbsolutePath()));
        ...
        cmd.add("--assetsDir");
        cmd.add(new File(system.installDir, "assets").getAbsolutePath());
        cmd.add("--assetIndex");
        cmd.add(system.version);
        cmd.add("--tweakClass");
        ...
        try {
            final Process proc = rt.exec(cmd.toArray(new String[0]), null, system.installDir);
